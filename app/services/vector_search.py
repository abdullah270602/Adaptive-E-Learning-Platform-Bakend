...
async def _perform_search_with_retry(
    ...
    for attempt in range(max_retries):
        try:
            ...
            logger.warning(
                f"Search attempt {attempt + 1} failed: {e}. Retrying in {delay}s..."
            )
...
def _validate_and_sanitize_inputs(
    embedded_query: List[float], user_id: str, top_k: int
) -> tuple:
...
def _process_and_validate_results(search_results: List, user_id: str) -> List[Dict]:
    ...
    for i, result in enumerate(search_results):
        try:
            ...
            if score is None:
                ...
                logger.warning(
                    f"Invalid score type for result {i}, converting to float"
                )
                try:
                    score = float(score)
    ...
    logger.info(
        f"Processed {len(chunks)} valid chunks out of {len(search_results)} results for user {user_id}"
    )
    return chunks
...
async def search_similar_chunks(
    ...
    try:
        ...
        embedded_query, user_id, top_k = _validate_and_sanitize_inputs(
            embedded_query, user_id, top_k
        )
