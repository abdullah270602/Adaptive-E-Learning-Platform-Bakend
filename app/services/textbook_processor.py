...
async def create_textbook_collections(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to create textbook collections.")
...
async def process_toc_pages(
    ...
    """
    Full pipeline: Extract TOC images → Process with LLaMA → Create textbook collection.
    """
    try:
        # # Step 1: Convert selected TOC pages into images
        # images = await prepare_toc_images(pdf_path, start_page, end_page)

        # Step 1: Extract raw text from PDF TOC pages
        text = extract_text_from_pdf(pdf_path, start_page, end_page)

        # Step 2: Extract structured TOC via LLM
        toc_structure = await process_toc_with_llm(text)

        # Step 3: Generate and upload collections metadata
        result = await create_textbook_collections(
            file_id=file_id,
        ...
        print("🐍 File: services/textbook_processor.py | Line: 267 | undefined ~ result",result)
        ...
        logging.error(f"[TOC] Failed in process_toc_pages: {e}")
        raise
