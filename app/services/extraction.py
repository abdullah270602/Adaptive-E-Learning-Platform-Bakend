...
def remove_repeated_lines(text: str, min_repeats: int = 3) -> str:
    ...
    return "\n".join(
        line
        for line in lines
        if counts[line.strip()] < min_repeats or line.strip() == ""
    )
...
def remove_noise_patterns(text: str) -> str:
    ...
    text = re.sub(
        r"(?i)(Figure|Table|Fig\.?)\s*\d+(\.\d+)*\s*[:.\-–—]?\s*.*?(?=\n|$)", "", text
    )

    # Remove page numbers and references
    text = re.sub(
        r"\b\d{1,4}\b(?=\s*$)", "", text, flags=re.MULTILINE
...
def merge_broken_lines(text: str) -> str:
    ...
    for i, line in enumerate(lines):
        ...
        if (
            ...
            and lines[i + 1].strip()
            and not re.match(r"^[A-Z]", lines[i + 1].strip())
...
def split_into_semantic_chunks(text: str) -> str:
    ...
    text = re.sub(
        r"(\n|^)([A-Z][^.!?]*(?:Algorithm|Method|Approach|Definition|Theorem|Lemma|Proof))",
        r"\1\n\2",
        text,
    )
...
def filter_content_for_mcq(text: str) -> str:
    ...
    for line in lines:
        ...
        special_char_ratio = sum(1 for c in line if not c.isalnum() and c != " ") / max(
            len(line), 1
        )
...
def enhance_meaningful_content(text: str) -> str:
    ...
    for paragraph in paragraphs:
        ...
        meaningful_word_count = sum(
            1 for word in words if word not in stop_words and len(word) > 2
        )

        if (
            len(words) > 0 and meaningful_word_count / len(words) > 0.3
        ):  # At least 30% meaningful words
